...
export async function POST(req: NextRequest) {
  ...
  if (new Set(ids).size !== ids.length) {
    return NextResponse.json({ error: 'Sama auðkenni kemur oftar en einu sinni fyrir.' }, { status: 400 });
  }
  const existingIds = new Set(gallery.map(p => p.id));
  const missing = gallery.filter(p => !ids.includes(p.id)).map(p => p.id);
  if (missing.length > 0) {
    return NextResponse.json({ error: `Auðkenni mynda vantar: ${missing.join(', ')}` }, { status: 400 });
  }
  const unknown = ids.filter(id => !existingIds.has(id));
  if (unknown.length > 0) {
    return NextResponse.json({ error: `Óþekkt auðkenni mynda: ${unknown.join(', ')}` }, { status: 400 });
  }
