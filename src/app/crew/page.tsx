'use client';

import '@/app/badlands.css';
import '@/app/board.css';
import { useEffect, useState } from 'react';
import Link from 'next/link';
import { formatAge } from '@/lib/format';
import type { FeedPost } from '@/app/api/crew/feed/route';
import AddressBar from '@/components/badlands/AddressBar';
import Footer from '@/components/badlands/Footer';
import PlayerHead from '@/components/badlands/PlayerHead';
import { ArrowIcon } from '@/components/badlands/Bits';
import { PAGE_LINKS } from '@/components/badlands/data';

interface CrewSummary { username: string; bio: string; photoCount: number; postCount: number; lastPost: string | null }

export default function CrewPage() {
  const [crew, setCrew] = useState<CrewSummary[] | null>(null);
  const [feed, setFeed] = useState<FeedPost[]>([]);
  const [tab,  setTab]  = useState<'members' | 'feed'>('members');

  useEffect(() => {
    fetch('/api/crew').then(r => r.json()).then(setCrew).catch(() => setCrew([]));
    fetch('/api/crew/feed').then(r => r.json()).then(setFeed).catch(() => {});
  }, []);

  return (
    <div className="b-desk">
      <div className="j">
        <div className="b-grain" aria-hidden="true" />
        <AddressBar links={PAGE_LINKS} always />
        <main className="b-wrap b-page">
          <Link href="/" className="b-back"><ArrowIcon flip /> aftur á forsíðu</Link>
          <div className="b-page__head">
            <div>
              <h1 className="b-title">Hópurinn</h1>
              <p className="b-lede">Öll sem hafa aðgang. Á hverri síðu eru kynning, tölur úr leiknum, færslur og myndir.</p>
            </div>
            <div className="b-tabs" style={{ marginBottom: 0 }} role="tablist">
              {(['members', 'feed'] as const).map(t => (
                <button key={t} type="button" role="tab" aria-selected={tab === t} className={`b-tab${tab === t ? ' is-active' : ''}`} onClick={() => setTab(t)}>
                  {t === 'members' ? 'Félagar' : `Nýjustu færslur${feed.length ? ` (${feed.length})` : ''}`}
                </button>
              ))}
            </div>
          </div>

          {tab === 'members' && (
            crew === null ? <p className="b-empty">sæki félagalistann…</p> :
            <div className="b-rollcall">
              {crew.map(m => (
                <div key={m.username}>
                  <Link href={`/crew/${m.username}`} className="b-paper b-poster b-poster--crew">
                    <span className="b-paper__nail" aria-hidden="true" />
                    <div className="b-poster__img"><PlayerHead name={m.username} size={128} /></div>
                    <div className="b-poster__name">{m.username}</div>
                    <div className="b-poster__note">{m.bio || (m.lastPost ? `skrifaði ${formatAge(m.lastPost)}` : 'ekkert heyrst enn')}</div>
                    <div className="b-poster__meta">{m.postCount} {m.postCount === 1 ? 'færsla' : 'færslur'} · {m.photoCount} {m.photoCount === 1 ? 'mynd' : 'myndir'}</div>
                  </Link>
                </div>
              ))}
            </div>
          )}

          {tab === 'feed' && (
            feed.length === 0 ? (
              <p className="b-empty">engar færslur enn. Félagar geta skrifað færslur á eigin síðu</p>
            ) : (
              <ul className="b-feed">
                {feed.map(post => (
                  <li key={post.id} className="b-paper b-feed__strip">
                    <span className="b-ledger__head" style={{ width: '2.2rem', height: '2.2rem' }}><PlayerHead name={post.username} size={64} /></span>
                    <div>
                      <div className="b-feed__meta">
                        <Link href={`/crew/${post.username}`} className="b-feed__who">{post.username}</Link>
                        <span>{formatAge(post.createdAt)}</span>
                      </div>
                      <p className="b-feed__text">{post.text}</p>
                    </div>
                  </li>
                ))}
              </ul>
            )
          )}
        </main>
        <Footer />
      </div>
    </div>
  );
}
