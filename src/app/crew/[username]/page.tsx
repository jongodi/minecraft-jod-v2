...
interface BadgeDef { id: string; label: string; check: (s: PlayerStat) => boolean }

const BADGE_DEFS: BadgeDef[] = [
  { id: 'played-10h',    label: '10h Played',      check: s => s.playTimeHours  >= 10    },
  { id: 'played-100h',   label: '100h Played',     check: s => s.playTimeHours  >= 100   },
  { id: 'played-500h',   label: '500h Played',     check: s => s.playTimeHours  >= 500   },
  { id: 'kills-100',     label: '100 Mob Kills',   check: s => s.mobKills       >= 100   },
  { id: 'kills-1k',      label: '1K Mob Kills',    check: s => s.mobKills       >= 1000  },
  { id: 'kills-5k',      label: '5K Mob Kills',    check: s => s.mobKills       >= 5000  },
  { id: 'walked-100km',  label: '100km Walked',    check: s => s.distanceWalked >= 100 * 100_000 },
  { id: 'walked-500km',  label: '500km Walked',    check: s => s.distanceWalked >= 500 * 100_000 },
  { id: 'deaths-10',     label: '10 Deaths',       check: s => s.deaths         >= 10    },
  { id: 'deaths-50',     label: '50 Deaths',       check: s => s.deaths         >= 50    },
  { id: 'crafted-1k',    label: '1K Items Crafted',check: s => s.itemsCrafted   >= 1000  },
  { id: 'pvp',           label: 'PvP',             check: s => s.playerKills    >= 1     },
];

function AchievementBadges({ stat }: { stat: PlayerStat }) {
  const earned = BADGE_DEFS.filter(b => b.check(stat));
