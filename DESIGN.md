...
| Dusk | The world | heimur | BlueMap fills the viewport directly under the mesas, so the badlands sit on top of the real world. The frame opens as a still (`public/map-poster.webp`, taken by `map:poster` at the start view); the viewer, an iframe with its own three.js dressed in the site's clothes (see *The 3D map* below), boots only when the lantern in the middle is pressed, and the still stays up only until the near low-detail layer is drawn (at most 0.6 s after the map has loaded), so the hand-over is one fade onto terrain rather than onto an empty sky; the detailed tiles then stream in while the map can already be moved. On phones the lantern makes the frame itself the whole screen, so the map's drag and pinch never fight the page scroll; back returns to the page, where the map rests as a still until the lantern is pressed again. A thin HUD lies over it: the title and the sync date, the heads of who is in, and the tools. the map opens at night, the hour the site is set in, so the town's own lights are what is lit, and *Nótt* (pressed while it is night) lets the sun back up; *Teiknað kort* lays the painted paper map over the same frame (`MapSheet`, the same `MapArt` the admin edits); *Myndir* opens the whole wall as an overlay in the admin's order; *Heill skjár* makes the frame the whole screen, HUD and places with it, so nothing reloads. While the viewer's own menu is open along the left edge, the HUD, the places and the postcard step aside. The eleven places ride a rail along the foot of the frame, each with its photo; a chip or a flag opens the place's postcard. A place given world coordinates in the map editor also stands in the 3D map as a lit lantern: its chip flies the camera there, its lantern opens its postcard, and its postcard offers the 3D map when the viewer isn't open yet. Places outside the rendered home area keep their postcards and simply have no lantern. |
...
The bar carries the mark, three doors and the address: Heimurinn, Eftirlýst, Hillan. The crew's door is named for its wanted board rather than for the crew, because a poster on a saloon wall is the more inviting sign and the portraits are inside either way. The first door is the world; the other two are rooms that open over it. One lantern is lit at a time, the door the visitor is behind: an open room's, otherwise the world's once the page has reached the frame; up at the sunset none is, even with a room left open below. A pressed door lights at once rather than waiting for the page to scroll down to it. The lit door also carries a rule in amber, under it in the bar and along its top edge at the foot of a phone. Over the sunset the bar is only its contents; once the page has scrolled it takes a surface. On phones the same three lanterns sit in a bar at the foot of the screen, in reach of a thumb, and the top bar keeps the mark and the address.
...
The world is drawn by BlueMap, and its viewer (`public/bluemap`, copied off the server by `npm run map:sync`) is BlueMap's own Vue app. It is never edited: every sync would copy BlueMap's files back over the changes. Instead the last step of the sync, `scripts/bluemap-brand.mjs` (also `npm run map:brand`), writes the three files BlueMap overwrites: an Icelandic `index.html` with JOÐ's title, icon, link preview and app manifest; `settings.json` with the map's versioned address, the start view from `MAP_START_VIEW` and view distances sized to the rendered world; and a language list with Icelandic first. Everything else that is JOÐ's lives beside BlueMap's files, where the sync never looks:

- `public/bluemap-jod/jod.css`, the skin. BlueMap's theme variables in the site's palette (its dark and default themes are the night, its light theme is paper), the three pixel faces, bevelled wood for the tools, the coordinates on a paper tag, square switches and sliders, the menu as a room with a plank and strata over it, player names on paper, and the places' lanterns. Linked in `<head>` after BlueMap's stylesheet, so BlueMap's own look never paints.
- `public/bluemap-jod/jod.js`, which waits for BlueMap's map and then holds the drawing to two device pixels per CSS pixel and a smaller hires area on phones, keeps the camera over the rendered world, softens the void under the horizon to the sky a shade down, and puts the plank across the full screen map. Inside the home page's frame it also answers the page: how far the first view has come, pause, night, fly to a place.
- `public/bluemap/lang/is.conf`, every string in the viewer in Icelandic, with its own controls help and a small credit to BlueMap.
...
| A poster before the viewer | No iframe, no WebGL and none of the viewer's assets until someone asks for them; a visitor who never touches the map never downloads it. Reaching for the lantern (hover, focus, touch) fetches the viewer's code ahead, and the poster leaves as soon as the terrain under it is drawn, never waiting for the detail: a visitor who asked for the map gets it at once. |
| The viewer wears the site's clothes | Every phone lands in the full screen map, so it has to be JOÐ's: Icelandic, the three pixel faces, wood for the tools, paper for names and coordinates, amber only on what can be pressed, and a plank with the way back. |
| Places stand in the 3D world | A place with coordinates is a lit lantern in the map and its chip flies there, so the rail and the world point at each other the way the walls and the world do. |
| The map is read under its sync | Every map file's address carries the sync's version, so the browser and the CDN keep it for good and a returning visitor reads the whole map from disk, while a new sync can never be answered with an old tile. |
| The viewer sleeps when it can't be seen | BlueMap redraws twenty times a second on its own; off screen, under a room, the album or the painted map it draws nothing and stops asking for players, so the page's own animations keep the GPU. |
| Full screen is the frame | Nothing reloads, the HUD and the places come along, and Escape or back returns the visitor exactly where they were. |
