...
Built with Next.js 15, TypeScript, and React.

The public site uses a world-atlas design: actual server screenshots, oversized JOÐ typography, stone-white and forest-green surfaces, and a four-part explorer. Places, the world map, datapacks, and crew stats each have their own focused view. The layout adapts to mobile and respects reduced-motion preferences.
...
- **Interactive map** : admin-managed world atlas with keyboard-selectable locations, zoom, and reset
...
Run `npm test` for the DOM interaction suite, `npm run lint` for linting, and `npm run build` for the production build. The interaction tests cover status recovery, clipboard failure, gallery navigation, datapack filters and persistence, map controls, and leaderboard sorting. They do not replace browser checks of responsive layout, native dialog focus behavior, or touch gestures.

Public UI components live in `src/components/atlas/` and their styles in `src/app/atlas.css`. The gallery is server-rendered from the same data used by the admin panel; the explorer reads the existing public APIs. Server failures display an unavailable state, and cached stats keep their snapshot date. Pack favorites are stored in this browser only.
...
2. Set `CREW_TOKEN_<UPPERCASE_USERNAME>` in your environment variables
